...
| Wedge cells "Direction A" (una cella cuneo che POSSIEDE lo spigolo concavo, 614 spigoli) | Implementato, scatta, scartato dal keep-best                                                                                                                              | `wedge_cells=False` è il default del converter **e** resta tale nel runner di produzione (`openfoam_runner.py`, chiamata `TetPolyDualConverter(...)` — non passa `wedge_cells`); nell'A/B della valvola non ha prodotto differenze (1.056 difetti in entrambi i casi). |
| `median_faces=True`                                                                    | Sposta il difetto: 852 piramidi → 766, ma skew 17 →**470**. Nessuno domina                                                                                        | Non è un default del converter: `median_faces=False`, e il runner di produzione lo lascia così (non lo passa esplicitamente). L'A/B più recente sulla valvola ha misurato 1.056 difetti residui con `True` contro 1.027 con `False` — cioè `True` è **peggio**, per questo il default resta `False` anche in produzione.                         |
