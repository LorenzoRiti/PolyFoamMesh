...
from polyfoammesh.gui.theme import (
    apply_theme, set_theme_mode, current_mode, current_request,
)
from polyfoammesh.gui.design_tokens import APP_NAME, APP_VERSION
from polyfoammesh.gui.branding import (
...
def _init_i18n(app):
    ...
    for p in paths:
        if translator.load(locale, "polyfoammesh", ".", str(p)):
...
def _load_plugins(app):
    ...
    # project root = app.py / src / polyfoammesh / .. = src/../.. = project root
...
def main():
    from polyfoammesh import __version__
    ...
    # `sys.executable -m polyfoammesh.core.feature_detector ...`, but in
    ...
    if len(_sys.argv) > 1 and _sys.argv[1] == "--feature-detect":
        from polyfoammesh.core.feature_detector import _main as _feature_detect_main
        _sys.exit(_feature_detect_main(_sys.argv[2:]))

    if len(_sys.argv) > 1 and _sys.argv[1] == "--watertight":
        from polyfoammesh.core.geometry_repair import _main as _watertight_main
    ...
    if len(_sys.argv) > 1 and _sys.argv[1] in _GMSH_FLAG_SUBCOMMAND:
        ...
        from polyfoammesh.core import gmsh_wrapper
