...
from polyfoammesh.core.feature_detector import FeatureDetectWorker
...
from polyfoammesh.core.gmsh_wrapper import gmsh_shutdown
...
from polyfoammesh.gui.constants import MAX_RECENT_STEP_FILES, MAX_STEP_FILE_BYTES
from polyfoammesh.gui.design_tokens import APP_VERSION, STATUS_READY
...
from polyfoammesh.gui.style import COLOR_DANGER, COLOR_PASS, COLOR_TEXT_DISABLED
from polyfoammesh.gui.task_runner import FunctionWorker, TaskManager
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        from polyfoammesh.core.disk_cleanup import auto_cleanup
    ...
    def _sync_theme_menu(self) -> None:
        from polyfoammesh.gui.theme import current_request
    ...
    def _setup_ribbon(self):
        ...
        def _make_ribbon_btn(text: str, icon_name: str, slot, checkable: bool = True) -> QToolButton:
            btn = QToolButton()
            btn.setText(text)
            from polyfoammesh.gui.branding import get_ribbon_icon
    ...
    def _load_geometry(self, shape: cq.Shape):
        ...
        def work(worker):
            ...
            heal_lines: list[str] = []
            try:
                from polyfoammesh.commercial.cad_healer import CADHealer
    ...
    def _start_watertight_check(self, meshes: list[trimesh.Trimesh]) -> None:
        ...
        from polyfoammesh.core.openfoam_runner import WatertightWorker
    ...
    def _check_watertight_sync(self, meshes: list[trimesh.Trimesh]) -> None:
        ...
        def work(worker):
            ...
            from polyfoammesh.core.geometry_repair import attempt_auto_repair
    ...
    def _on_cleanup_cases(self):
        ...
        from polyfoammesh.core.disk_cleanup import (
            auto_cleanup,
            cases_disk_usage_mb,
            list_cases,
        )
    ...
    def _start_native_poly_worker(self, meshes: list, my_id: int):
        """Run the experimental native cut-cell -> median-dual mesher."""
        from polyfoammesh.commercial.native_poly_bridge import (
            NativePolyParams,
            run_native_poly,
        )
    ...
    def _refinement_box_base_size(self) -> float:
        ...
        from polyfoammesh.core.refinement_boxes import base_bulk_size
    ...
    def _on_add_refinement_box(self):
        ...
        if not ok:
            return
        try:
            from polyfoammesh.core.refinement_boxes import (
    ...
    def _resolve_auto_mesher(self) -> str:
        ...
        try:
            from polyfoammesh.commercial.autopoly_bridge import create_mesher
    ...
    def _do_meshing_pipeline(
        ...
        from polyfoammesh.core.geometry import compute_bbox_full
        dx, dy, dz = compute_bbox_full(self._meshes)
    ...
    def _make_temp_geometry_for_gmsh(self) -> str | None:
        ...
        def _work(worker):
            ...
            if meshes:
                ...
                try:
                    from polyfoammesh.core.stl_writer import export_multisolid_stl
    ...
    def _start_gmsh_volume_worker(self, step_path: str, my_id: int):
        from polyfoammesh.core.openfoam_runner import GmshVolumeWorker
        logger.info(
        ...
        from polyfoammesh.core.case_setup import write_control_dict
    ...
    def _continue_gmsh_direct(self, my_id: int, msh_path: Path, names: list[str]):
        ...
        def worker_fn(worker):
            ...
            import subprocess
            import traceback
            from polyfoammesh.core.openfoam_runner import _gmsh_wrapper_script_cmd
            try:
    ...
    def _start_autopoly_worker(self, geom_path: str, my_id: int):
        ...
        from polyfoammesh.commercial.autopoly_bridge import (
            AutopolyParams,
            run_autopoly,
        )
    ...
    def _on_cell_count_found(self, count: int):
        from polyfoammesh.gui.design_tokens import SUCCESS
    ...
    def _on_export_mesh(self, fmt: str):
        ...
        from polyfoammesh.core.mesh_export import EXPORT_FORMATS
        ...
        from polyfoammesh.core.openfoam_runner import ExportWorker
    ...
    def _on_export_baramflow(self):
        ...
        from polyfoammesh.core.baramflow_export import export_case, validate_case
    ...
    def _on_export_pdf(self):
        if not self._case_dir:
            QMessageBox.warning(self, "No Mesh", "Generate a mesh first.")
            return
        from polyfoammesh.gui.pdf_report import MeshReportPDF
        path, _ = QFileDialog.getSaveFileName(
    ...
    def _on_bc_editor(self):
        if not self._case_dir:
            ...
            return
        from polyfoammesh.gui.bc_dialog import BCEditorDialog
    ...
    def _on_solver_setup(self):
        if not self._case_dir:
            QMessageBox.warning(self, "No Case", "Generate a mesh first.")
            return
        from polyfoammesh.commercial.bc_editor import BCEditor
    ...
    def _on_template_selector(self):
        from polyfoammesh.gui.template_selector import TemplateSelectorDialog
    ...
    def _on_quick_mesh(self):
        ...
        from polyfoammesh.commercial.quick_mesh import QuickMesh
        from polyfoammesh.core.geometry import compute_bbox_dim
        detail = self._params.get_detail_level()
    ...
    def _on_adaptive_mesh(self):
        ...
        if getattr(self, '_ooda_panel', None) is None:
            from polyfoammesh.gui.ooda_panel import OODAPanel
    ...
    def _on_solution_adaptive(self):
        ...
        from polyfoammesh.core.gmsh_subprocess import assemble_runnable_case
        from polyfoammesh.core.openfoam_runner import SolutionAdaptiveWorker
        from polyfoammesh.core.solution_adaptive import AdaptiveParams
        ...
        from polyfoammesh.core.boundary_reader import count_cells as _cc
        ...
        def _remesh(size_field: Path, cycle: int):
            from polyfoammesh.core.gmsh_subprocess import remesh_from_cad
