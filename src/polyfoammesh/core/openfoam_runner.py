...
def poly_geo_wall_patch_names(case_dir: Path | str) -> list[str]:
    ...
    try:
        from polyfoammesh.core.boundary_reader import parse_boundary
        from polyfoammesh.core.case_setup import infer_patch_roles
...
try:
    ...
    raise ImportError(
        "PySide6 is required for polyfoammesh. "
...
class QualityFixWorker(QObject):
    ...
    def _on_checkmesh_result(self, report):
        ...
        try:
            from polyfoammesh.commercial.quality_engine import QualityEngine, QualityMetrics
...
def _gmsh_wrapper_script_cmd(
    ...
    by file path rather than via `python -m polyfoammesh.core.<module>`.

    `-m` first fully imports the parent package (polyfoammesh.core),
...
class ExportWorker(QObject):
    ...
    def run(self):
        try:
            from polyfoammesh.core.mesh_export import export_mesh
